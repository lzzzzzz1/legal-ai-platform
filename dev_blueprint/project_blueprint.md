...
> **用户上传 `.docx` 合同 → 提取纯文本 → 调用千问百炼平台的 qwen-max 模型进行智能审查 → 前端展示风险卡片**
...
- **样式**: Vanilla CSS 或 CSS Modules (追求简洁现代 of UI，如果需要快速排版可用少量 Tailwind)
...
### Task 1: 仓库初始化与前后端骨架搭建 (已完成)
- [x] 创建后端 FastAPI 基础骨架 (包含 `main.py`, 跨域 CORS 配置，基本的 `/health` 接口)
- [x] 创建前端 Vite-React 基础骨架
- [x] 配置好前后端联调的脚本 (如 `npm run dev` 代理请求到 `localhost:8000`)

### Task 2: 核心链路 - 后端解析与 AI 调用 (进行中)
...
---
**收到请回复：“已理解全局蓝图与 MVP 约束，准备开始执行 Task 2”，并立刻着手编写后端解析与 AI 调用逻辑。**
